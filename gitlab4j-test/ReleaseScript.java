...
//DEPS https://github.com/gitlab4j/gitlab4j-api/tree/c6108d816e55511ae733dcb67c8b3b2e064b87e9
//JAVA 17
...
@Command(name = "ReleaseScript", mixinStandardHelpOptions = true, version = "ReleaseScript 0.1", description = "Tests for GitLab4J")
public class ReleaseScript implements Callable<Integer> {
    ...
    @Parameters(index = "0", description = "action to execute", defaultValue = "GET_RELEASES")
    private Action action;

    @Option(names = { "-p", "--project" }, description = "project")
    private String project;

    @Option(names = { "-r", "--release" }, description = "release tag name")
        ...
        try (GitLabApi gitLabApi = new GitLabApi(gitLabUrl, gitLabAuthValue)) {
            ...
            case GET_RELEASES:
                ...
                List<Release> releases = gitLabApi.getReleasesApi()
                        .getReleases(idOrPath(project));
                System.out.println(releases);
                break;
            case GET_RELEASE:
                ensureExists(project, "project");
                ensureExists(releaseTagName, "release");
                Release release = gitLabApi.getReleasesApi()
                        .getRelease(idOrPath(project), releaseTagName);
    ...
    public static void main(final String... args) {
        final int exitCode = new CommandLine(new ReleaseScript()).execute(args);
